...
try:
    with open('Registro de alterações.csv', encoding='latin_1', mode='r', newline='') as criar:
        criar.seek(0)
        leitura = csv.reader(criar)

except FileNotFoundError:
    colunas = ['Tipo combustível;Novo valor por litro;Data da alteração']
    with open('Registro de alterações.csv', encoding='latin_1', mode='a+', newline='') as criar_planilha:
        criar_planilha.seek(0)
        criar_planilha_csv = csv.writer(criar_planilha)
        criar_planilha_csv.writerow(colunas)
...
class Posto_combustivel():
    ...
    def alterar_valor_combustivel(self,novo_valor_combustivel):
        ...
        log_alteracoes = dict([(self.tipo_combustive,self.valor_por_litro)])

        with open('Registro de alterações.csv', encoding='latin_1', mode='a+', newline='') as arquivo:
            arquivo.seek(0)
            escrever = csv.writer(arquivo)
            for x,y in log_alteracoes.items():
                escrever.writerow([f'{x};{y};{data_atual}'])
