...
if('n' in conta.lower()):
    ...
    while(True):
        ...
        print(" ",end='\n')
        if(not comprimento.isalpha() and not largura.isalpha() and '-' not in comprimento and '-' not in largura):
    ...
    while(True):
        ...
        if (not area_usuario.isalpha() and '-' not in area_usuario):
...
# c-) misturar latas e galões, de forma que o desperdício de tinta seja menor.
#   Acrescente 10% de folga e sempre arredonde os valores para cima, isto é, considere latas cheias.
#Obs.: 1 litro para cada 6 metros quadrados. Ou seja 1L/ 6m²


if('n'in conta.lower()):
    litros_tinta = 0.1 if round((area_calculada/6),0)*1.10<=0.1 else round((area_calculada/6),0)*1.10
    area_usada = 0.1 if area_calculada <= 0.1 else round(area_calculada,2)
else:
    litros_tinta = 0.1 if round((area_usuario / 6), 0)*1.10 <= 0.1 else round((area_usuario / 6), 0)*1.10
...
if(litros_tinta >= 2 and litros_tinta <= 10):
    cenario_a_latas = 1
    cenario_b_latas = round((litros_tinta / 3.6), 0)
    cenario_c_latas = 0
    ...
    cenario_a_latas = round((litros_tinta / 18), 0) + litros_tinta % 18
    cenario_b_latas = round((litros_tinta / 3.6), 0)
...
print(valores)
print(cenarios[verificar], "cenários[verificar]")
